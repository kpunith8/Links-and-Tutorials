...
// understanding this, returning callbacks from a function
function FileObject() {
  this.fileName = '';
...
### Create a node package locally

Project should have `package.json` with the following details
```json
"name": "Sample Project",
"description": "Description of the project"s,
"version": '1.0.0',
"main": "lib/index.js",
"private": true
```
...
First add the user in the CLI `$ npm adduser`, once the user added publish the package using `npm publish`,
make sure `private` property is removed in the  `package.json`

## Advanced nodeJS: Samer Buna

### Architecture

Check the process version of v8
```
$ node -p 'process.verisons.v8'

$ node --harmony ## will load staged feature of the node

$ node --v8-options | grep "in progress" ## List in progress node features
```
...
Node has `libuv`,  C library, used to abstract `non-io blocking operations`, it handles
child processes, TCP/UDP sockets, file system includes thread pool to handle
async operations, provides event loop.
...
Buffer: To work with binary streams of data chunck of data allocated outside of V8
```js
...
`> os.` hit tab to auto complete all possible methods available.
Usage: `os.freemem()` to display free memory and `os.type()`
to display the type of Operating System.

### fs module

Has both synchronous and asynchronous flavours on all methods
  ```javascript
  const fs = require('fs');
...
Debug an node file using, `node debug file.js` it opens debugger in node REPL,
type `help` to get the available options to debug
